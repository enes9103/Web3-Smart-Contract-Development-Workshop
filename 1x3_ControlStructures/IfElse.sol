...
contract IfElse {
    ...
    constructor(string memory _password) {
        hashedPassord = keccak256(abi.encode(_password));       // burası girilen şifreyi hash *** yapan kod bloğu
    }

    function login(string memory _password) public returns (bool) {
        if (hashedPassord == keccak256(abi.encode(_password))) {    // burası girilen şifreyi hash *** yapan kod bloğu
            loginCount++;
            //loginCount += 1;
            return true;
        } else {
        ...
        // Üstteki if else yapısı yeri aşağıdaki eşitlik sorgulamasını kullanabiliriz.
    ...
    function loginlogin(string memory _password) public view returns (uint256) {
        ...
        // Yukarıda ki if else yapısını JS teki gibi ternary yapısı ile (altta) kullanabiliriz.
